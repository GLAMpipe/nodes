var nodeid = "#export-data-dspace-addfile";
...
$(nodeid + "_url").change(function() {
	add_div.hide();
	$(nodeid + "_status").empty().append("Trying to connect...")
	function success() {
		$(nodeid + "_status").empty().append("DSpace responded OK").addClass("alert alert-success");
		$(nodeid + "_hidden").show()
	};
	function fail(msg) {
		$(nodeid + "_status").empty().append("Connection failed!")
		alert("DSpace rest API not working!")
	};
	testUrl($(nodeid + "_url :selected").val(), success, fail);
});
