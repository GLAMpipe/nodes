var nodeid = "#source-web-dspace-collection";
...
$("#source-web-dspace-collection_url").change(function() {
	add_div.hide();
	function success() {
		$("#source-web-dspace-collection_status").empty().append("DSpace responded OK").addClass("alert alert-success");
	};
	function fail(msg) {alert("DSpace rest API not working!")};
	testUrl($("#source-web-dspace-collection_url :selected").val(), success, fail);
});
