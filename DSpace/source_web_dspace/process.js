...
if (core.response && core.response.status == 200 ) {
	// count query rounds
	c.vars.round_counter++;

	if (core.response.data.items && core.response.data.items.length > 0) {

		out.say("progress", "procesed so far " + context.vars.record_counter );
		var imports = [];
		var updates = [];

		for (var i = 0; i < core.response.data.items.length; i++) {

			// count records
			context.vars.record_counter++;
			imports.push(makeRecord(core.response.data.items[i])); // this adds some fields to the original data
			...
			if(context.node.settings.mode === "update" && context.node.settings.update_key) {
				if(!context.records.includes(core.response.data.items[i][context.node.settings.update_key])) {
					updates.push(makeRecord(core.response.data.items[i]));
